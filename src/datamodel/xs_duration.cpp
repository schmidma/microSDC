...
void Duration::parse(const std::string& string)
{
  ...
  if (match[2].matched)
  {
    years_ = Years{std::atoi(match[2].str().c_str())};
  }
  if (match[3].matched)
  {
    months_ = Months{std::atoi(match[3].str().c_str())};
  }
  if (match[4].matched)
  {
    days_ = Days{std::atoi(match[4].str().c_str())};
  }
  if (match[5].matched)
  {
    hours_ = std::chrono::hours{std::atoi(match[5].str().c_str())};
  }
  if (match[6].matched)
  {
    minutes_ = std::chrono::minutes{std::atoi(match[6].str().c_str())};
  }
  if (match[7].matched)
  {
    seconds_ = std::chrono::duration<float, std::chrono::seconds::period>{
        std::atof(match[7].str().c_str())};
  }
}
